...
const ProductCard = () => {
    ...
    async function getProductById(productid) {
        await axios.get("http://localhost:8082/product/get/by-id?id="+productid)
            .then((response) => {
  ...
  return (
      <div className='flex flex-col justify-between lg:flex-row gap-16 lg:items-center max-w-7xl mx-auto p-8'>
          <div className='flex flex-col gap-6 lg:w-2/4'>
              <img src={productdata?.imagePath} alt="" className='w-full h-full aspect-square object-cover rounded-xl'/>
          ...
          <div className='flex  flex-col gap-4 lg:w-2/4'>
              <div>
                  <span className=' text-emerald-300 font-semibold'>{productdata?.brandDto.brandName}</span>
              ...
              <h6 className='text-2xl font-semibold'>Rs.{productdata?.discountPrice}</h6>
              ...
              <div className='flex flex-row items-center gap-12'>
                  ...
                  h-full hover:bg-transparent hover:border-2 border-[#f8717199] hover:text-[#221f1f]'>Proceed To Buy</button>
              </div>
          </div>
      </div>
  );
