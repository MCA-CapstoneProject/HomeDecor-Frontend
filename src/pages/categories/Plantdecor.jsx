...
import { cartState } from "../../features/product/cart.slice";
...
const Plantdecor = () => {
  ...
  const { listcart } = useSelector(cartState);
  ...
  const handleCartToggle = (id) => {
    if (!listcart || !listcart.includes(id)) {
      cartHelper.addToCart(id, userId);
      cartHelper.addToArrayCartlist(id);
    } else {
      cartHelper.removeFromArrayCartlist(id);
      cartHelper.deleteFromCart(id);
    }
  };
              ...
              sortedList.map((item) => (
                ...
                >
                  ...
                  <div className="p-7">
                    ...
                    <button
                      onClick={() => handleCartToggle(item.productId)}
                      className="mt-4 w-full text-[#9d6a37] border-[1.5px] 
                            border-[#9d6a37] px-3 py-2 rounded uppercase hover:bg-[#9d6a37] hover:text-[#fff]"
                    >
                      {list && list.includes(item.productId)
